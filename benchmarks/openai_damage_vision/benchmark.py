"""Standalone OpenAI vision benchmark for the CarCal damage-segmentation task.

Sends each test image to the OpenAI Responses API and asks it to segment
visible vehicle damage into the same 6 classes as the ground-truth COCO file
(instances_test2017.json: 1 dent, 2 scratch, 3 crack, 4 glass shatter,
5 lamp broken, 6 tire flat). Predictions are converted into the exact
EngineAnalyzeResponse / Detection schema that Engine/Engine_methods.py's
process_damage_only() returns in production (see root models.py), so the
two can later be scored with the same evaluation code.
...
Usage:
    python benchmark.py --images-dir <dir> --coco-json <path> --output results.jsonl --test
    python benchmark.py --images-dir <dir> --coco-json <path> --output results.jsonl
"""
...
from schema import DAMAGE_CLASSES, PROMPT_TEXT, PROMPT_VERSION, RESPONSE_SCHEMA
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--coco-json", required=True, help="Path to instances_test2017.json")
...
def setup_logger(log_file: Path) -> logging.Logger:
    logger = logging.getLogger("openai_damage_benchmark")
...
def load_coco_images(coco_json_path: str) -> list[dict]:
    with open(coco_json_path, "r", encoding="utf-8") as f:
        coco = json.load(f)
    images = coco.get("images", [])
    if not images:
        raise ValueError(f"No 'images' entries found in {coco_json_path}")
...
def write_results(output_path: Path, results: dict[int, dict]) -> None:
    """Rewrites the whole file atomically on every image, keyed by imageId.
    374 images is small enough that this is simpler and safer than
    streaming appends (no duplicate lines on --retry-errors, and the file
    is always valid JSONL even if the process is killed mid-run)."""
...
def process_image(
    ...
    coco_w, coco_h = img_meta.get("width"), img_meta.get("height")
    if coco_w and coco_h and (coco_w != width or coco_h != height):
        logger.warning(
            f"[{file_name}] COCO dims {coco_w}x{coco_h} != actual {width}x{height}; using actual dims"
        )
...
def main() -> None:
    ...
    images = load_coco_images(args.coco_json)
    if args.test:
        images = images[:3]
